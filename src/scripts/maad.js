...
async function requestAmbulance(NodeId) {
  ...
  <div>
  <img src="/home/kravlone/Documents/Programs_Projects/Projects/Ambulance_Management_And_Routing_System/src/assets/ambulance.png" alt="ambulance">
  </div>
...
function animateAmbulanceOnPath(path) {
    const ambulanceMarker = document.createElement("div");
    ambulanceMarker.className = "ambulance-marker";
    document.body.appendChild(ambulanceMarker);
    path.forEach((nodeId, index) => {
        setTimeout(() => {
            const area = document.querySelector(`area[alt='${nodeId}']`);
            if (area) {
                ambulanceMarker.style.top = `${area.coords.split(",")[1]}px`;
                ambulanceMarker.style.left = `${area.coords.split(",")[0]}px`;
            }
        }, index * 500);
    });
}
